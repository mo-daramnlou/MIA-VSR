...
class WGEN23VSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels=24, num_blocks=6, out_channels=3, integrate_channels=16):
        ...
        """
        super(WGEN23VSR, self).__init__()
        self.scale = scale
        self.integrate_channels=integrate_channels

        # Feature extraction layer
        self.fea_conv = nn.Conv2d(3*in_channels, mid_channels, kernel_size=3, padding=1)
    ...
    def forward(self, lqs):
        ...
        if self.training:
            blq = lqs_batch.view(n, t, 3, h, w).contiguous()
            # Concatenate the zero_frame at the beginning with the shifted_frames
            shifted_blq = torch.cat((blq[:,0:1,:,:,:], blq[:,:-1,:,:,:]), dim=1)
            shifted_blq = shifted_blq.view(n*t, 3, h, w).contiguous()

            alq = lqs_batch.view(n, t, 3, h, w).contiguous()
            # Concatenate the zero_frame at the beginning with the shifted_frames
            shifted_alq = torch.cat((alq[:,1:,:,:,:], alq[:,t-1:t,:,:,:]), dim=1)
            shifted_alq = shifted_alq.view(n*t, 3, h, w).contiguous()

        else:
            # Concatenate the zero_frame at the beginning with the shifted_frames
            shifted_blq = torch.cat((lqs_batch[0:1], lqs_batch[:-1]), dim=0)

            # Concatenate the zero_frame at the beginning with the shifted_frames
            shifted_alq = torch.cat((lqs_batch[1:], lqs_batch[t-1:t]), dim=0)

        lq = torch.cat([shifted_blq,lqs_batch,shifted_alq],dim=1)

        # Assert
        if self.training:
            blq = lqs_batch.view(n* t, 3, h, w).contiguous()
            alq = lqs_batch.view(n* t, 3, h, w).contiguous()
            for i,f in enumerate(lq):
                if i%t == 0:
                    assert torch.equal(f[0:3], blq[i]), ('ass failed1')
                else:
                    assert torch.equal(f[0:3], blq[i-1]), ('ass failed2')

                if i%t == t-1:
                    assert torch.equal(f[-3:], alq[i]), ('ass failed3')
                else:
                    assert torch.equal(f[-3:], alq[i+1]), ('ass failed4')
        else:
            for i,f in enumerate(lq):
                if i == 0:
                    assert torch.equal(f[0:3], lqs_batch[i]), ('ass failed1')
                else:
                    assert torch.equal(f[0:3], lqs_batch[i-1]), ('ass failed2')

                if i == len(lq)-1:
                    assert torch.equal(f[-3:], lqs_batch[i]), ('ass failed3')
                else:
                    assert torch.equal(f[-3:], lqs_batch[i+1]), ('ass failed4')

        # t, c, h, w = lqs.shape
        image_skip = lqs_batch
        # Feature extraction
        x = self.relu(self.fea_conv(lq))
        feat_skip=x
...
if __name__ == '__main__':
    ...
    model = WGEN23VSR(mid_channels=24, num_blocks=6)
    model.eval()
    ...
    # edge_model.export("/content/MIA-VSR/assets/wgen20vsr2.tflite")