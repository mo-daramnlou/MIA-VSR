...
class WGEN8VSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels=28, num_blocks=4, out_channels=3, integrate_channels=28):
        ...
        for _ in range(num_blocks):
            ...
            middle_layers.append(nn.Conv2d(mid_channels, mid_channels, kernel_size=3, padding=1, groups= mid_channels))
            middle_layers.append(nn.Conv2d(mid_channels, mid_channels, kernel_size=1))
            middle_layers.append(nn.ReLU(inplace=True))
...
if __name__ == '__main__':
    ...
    model = WGEN8VSR(mid_channels=28, num_blocks=4)
    model.eval()
