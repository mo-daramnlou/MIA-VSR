...
class WGEN55VSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels=24, num_blocks=4, out_channels=3, integrate_channels=16):
        ...
        self.fea_conv_ds4 = nn.Conv2d(in_channels, 8, kernel_size=3, padding=1, stride=4)
        ...
        self.rconv= nn.Conv2d(mid_channels + 16 , mid_channels, kernel_size=1)
    ...
    def forward(self, lqs):
        ...
        # Middle convolutions ds4
        x_ds4 = self.relu(self.fea_conv_ds4(lqs_batch))
        x_ds4 = F.interpolate(x_ds4, scale_factor=4, mode='nearest')

        # Feature extraction
        x = self.relu(self.fea_conv(lqs_batch))

        x = torch.cat([x,x_ds2, x_ds4],dim=1)
...
if __name__ == '__main__':
    ...
    model = WGEN55VSR(mid_channels=24, num_blocks=4)
    model.eval()
    ...
    # edge_model.export("/content/MIA-VSR/assets/wgen51vsr2.tflite")