...
class WGEN46VSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels=24, num_blocks=4, out_channels=3, integrate_channels=16):
        ...
        # T convs
        # self.tconv1 = nn.Conv2d(3 * integrate_channels, out_channels * (scale**2), kernel_size=1)
        self.tconv2 = nn.Conv2d(out_channels * (scale**2), out_channels * (scale**2), kernel_size=3, padding=1)
        self.tconv3 = nn.Conv2d(out_channels * (scale**2), out_channels * (scale**2), kernel_size=3, padding=1)
...
if __name__ == '__main__':
    ...
    model = WGEN46VSR(mid_channels=28, num_blocks=4)
    model.eval()
