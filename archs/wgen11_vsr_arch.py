...
class WGEN11VSR(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        # Custom initialization for tconv1 weights
        with torch.no_grad():
            # Create a new weight tensor of the same shape, initialized to zeros
            new_weights = torch.zeros_like(self.tconv1.weight)
            
            # The input to tconv1 is torch.cat([shifted_btx, ptx, shifted_atx], dim=1)
            # 'ptx' corresponds to the middle channels.
            start_idx = self.integrate_channels
            end_idx = 2*self.integrate_channels

            # Set the weights for the 'ptx' input channels to 1
            new_weights[:, start_idx:end_idx, :, :] = 1.0

            # Assign the new custom weights to the tconv1 layer
            self.tconv1.weight.copy_(new_weights)
...
if __name__ == '__main__':
    ...
    model = WGEN11VSR(mid_channels=28, num_blocks=4)
    model.eval()
