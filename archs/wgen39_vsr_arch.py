...
class WGEN39VSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels=24, num_blocks=6, out_channels=3, integrate_channels=16):
        ...
        middle_layers = []
        self.middle_conv1 = nn.Conv2d(mid_channels , mid_channels - 8 , kernel_size=3, padding=1)
        for _ in range(int(num_blocks/2)-1):
            middle_layers.append(nn.Conv2d(mid_channels - 8, mid_channels - 8 , kernel_size=3, padding=1))
            middle_layers.append(nn.ReLU(inplace=True))
        self.middle_convs1 = nn.Sequential(*middle_layers)

        self.middle_conv2 = nn.Conv2d(mid_channels + 8 , mid_channels , kernel_size=3, padding=1)
        middle_layers1 = []
        for _ in range(int(num_blocks/2)-1):
        ...
        self.middle_conv_ds = nn.Conv2d(mid_channels, 16, kernel_size=3, padding=1, stride=2)
        middle_layers_ds1 = []
        for _ in range(int(num_blocks/2)-1):
            middle_layers_ds1.append(nn.Conv2d(16, 16, kernel_size=3, padding=1))
            middle_layers_ds1.append(nn.ReLU(inplace=True))
        self.middle_convs_ds2 = nn.Sequential(*middle_layers_ds1)
    ...
    def forward(self, lqs):
        ...
        # Middle convolutions1 ds
        x_ds = self.relu(self.middle_conv_ds(x))
        feat_skip_ds= x_ds
        x_ds = self.middle_convs_ds2(x_ds)
        x_ds = x_ds + feat_skip_ds
        x_ds = F.interpolate(x_ds, scale_factor=2, mode='nearest')

        # Middle convolutions1
        x = self.relu(self.middle_conv1(x))
        feat_skip=x
        ...
        x = self.relu(self.middle_conv2(x))
        feat_skip=x
...
if __name__ == '__main__':
    ...
    model = WGEN39VSR(mid_channels=24, num_blocks=6)
    model.eval()
    ...
    # edge_model.export("/content/MIA-VSR/assets/wgen39vsr.tflite")