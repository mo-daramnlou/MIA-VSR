...
def create_reorder_conv():
    """Layer 1: 1x1 conv to reorder (48 -> 48)"""
    conv = nn.Conv2d(48, 48, 1, bias=False)
    W = torch.zeros(48, 48)
    for k in range(48):
        p_w, p_h = k % 2, (k // 2) % 2; C, g_w = (k // 4) % 3, (k // 12) % 2
        g_h = k // 24; r_h, r_w = g_h * 2 + p_h, g_w * 2 + p_w
        i = C * 16 + r_h * 4 + r_w; W[k, i] = 1.0
    conv.weight.data = W.view(48, 48, 1, 1); conv.weight.requires_grad = False
    return conv

def create_upsample_conv():
    """Layer 3: 1x1 conv to upsample/broadcast (12 -> 48)"""
    conv = nn.Conv2d(12, 48, 1, bias=False)
    W = torch.zeros(48, 12)
    for k in range(48):
        C = k // 16; spatial_idx = k % 16
        r_h, r_w = spatial_idx // 4, spatial_idx % 4
        g_h, g_w = r_h // 2, r_w // 2
        j = (g_h * 2 + g_w) * 3 + C; W[k, j] = 1.0
    conv.weight.data = W.view(48, 12, 1, 1); conv.weight.requires_grad = False
    return conv


@ARCH_REGISTRY.register()
class WGEN62VSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels=24, num_blocks=4, out_channels=3, integrate_channels=16):
        ...
        # T convs ds
        self.orderconv = create_reorder_conv()
        # self.tconv_ds1 = nn.Conv2d(mid_channels + 2 * integrate_channels, out_channels * (scale), kernel_size=1)
        # self.tconv_ds2 = nn.Conv2d(out_channels * (scale), out_channels * (scale), kernel_size=3, padding=1)
        self.tconv_ds3 = nn.Conv2d(out_channels * (scale**2), out_channels * (scale), kernel_size=1, groups=4)
        self.branch_scale = nn.Parameter(torch.ones(48))
        self.upsampleconv = create_upsample_conv()
        ...
        self.psconv.weight.data[:, -3:, :, :].fill_(1.0)
    ...
    def forward(self, lqs):
        ...
        # T convs ds
        # tx_ds = self.relu(self.tconv_ds1(x))
        # tx_ds = self.relu(self.tconv_ds2(tx_ds))
        # tx_ds = self.relu(self.tconv_ds3(tx_ds))

        # T convs
        tx = self.relu(self.tconv1(x))
        tx = self.relu(self.tconv2(tx))

        tx_ds = self.orderconv(tx)
        tx_ds = self.relu(self.tconv_ds3(tx_ds))
        tx_ds = self.upsampleconv(tx_ds)

        scale = self.branch_scale.view(1, -1, 1, 1)
        tx_ds = tx_ds * scale
        ...
        x = torch.cat((tx + tx_ds, image_skip), dim=1)
...
if __name__ == '__main__':
    ...
    model = WGEN62VSR(mid_channels=24, num_blocks=4)
    model.eval()
    ...
    # edge_model.export("/content/MIA-VSR/assets/wgen51vsr5.tflite")