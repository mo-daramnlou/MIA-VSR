...
class RGEN4INFERVSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels=28, num_blocks=4, out_channels=3):
        ...
        """
        super(RGEN4INFERVSR, self).__init__()
        self.scale = scale
        ...
        self.aconv3 = nn.Conv2d(mid_channels, mid_channels, kernel_size=3, padding=1)
        # self.aconv4 = nn.Conv2d(mid_channels, mid_channels, kernel_size=3, padding=1)
    ...
    def forward(self, lqs):
        ...
        for i in range(1, t):
            ...
            now_frame_backward = x[t-1-i:t-i, :, :, :]
            now_frame = torch.cat([now_frame_forward, now_frame_backward], dim=0)
            
            # print(now_frame.shape)
            # print(hidden.shape)

            # combined = torch.cat([hidden.unsqueeze(2), now_frame.unsqueeze(2)], dim=2)
            # # Shape is now (1, 28, 2, H, W)
            # # 2. Reshape to the final interleaved format
            # m = combined.view(2, 56, h, w)

            m = torch.cat([hidden, now_frame], dim=1)
            hidden = self.relu(self.aconv1(m))
            hidden = self.relu(self.aconv2(hidden))
            hidden = self.relu(self.aconv3(hidden))
            res.append(hidden)
        ...
        for i in range(0, t):
            ...
            fused_features.append(torch.cat([res[i][:n, :, :, :], res[t-1-i][n:, :, :, :]], dim=1))
...
if __name__ == '__main__':
    ...
    model = RGEN4INFERVSR(mid_channels=28, num_blocks=4)
    model.eval()
