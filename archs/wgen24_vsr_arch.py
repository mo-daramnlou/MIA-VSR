...
class WGEN24VSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels1=16, mid_channels2=24, num_blocks=6, out_channels=3, integrate_channels1=8, integrate_channels2=16):
        """
        ...
        """
        super(WGEN24VSR, self).__init__()
        self.scale = scale
        self.integrate_channels=integrate_channels2

        # Feature extraction layer
        self.fea_conv = nn.Conv2d(in_channels, mid_channels1, kernel_size=3, padding=1)

        # Middle convolutional layers1
        self.mconv1 = nn.Conv2d(mid_channels1, mid_channels1, kernel_size=3, padding=1)
        self.mconv2 = nn.Conv2d(mid_channels1, mid_channels1, kernel_size=3, padding=1)
        self.mconv3 = nn.Conv2d(mid_channels1, mid_channels1, kernel_size=3, padding=1)
        self.mconv4 = nn.Conv2d(mid_channels1, mid_channels1, kernel_size=3, padding=1)

        # Pre M convs
        self.pmconv2 = nn.Conv2d(mid_channels1, mid_channels1, kernel_size=1)
        self.pmconv3 = nn.Conv2d(mid_channels1, mid_channels1, kernel_size=3, padding=1)
        self.pmconv4 = nn.Conv2d(mid_channels1, mid_channels1, kernel_size=3, padding=1)

        # bM convs
        # self.btconv1 = nn.Conv2d(mid_channels, integrate_channels, kernel_size=1)
        self.bmconv2 = nn.Conv2d(mid_channels1, integrate_channels1, kernel_size=1)
        self.bmconv3 = nn.Conv2d(integrate_channels1, integrate_channels1, kernel_size=3, padding=1)
        self.bmconv4 = nn.Conv2d(integrate_channels1, integrate_channels1, kernel_size=3, padding=1)

        # aT convs
        # self.atconv1 = nn.Conv2d(mid_channels, integrate_channels, kernel_size=1)
        self.amconv2 = nn.Conv2d(mid_channels1, integrate_channels1, kernel_size=1)
        self.amconv3 = nn.Conv2d(integrate_channels1, integrate_channels1, kernel_size=3, padding=1)
        self.amconv4 = nn.Conv2d(integrate_channels1, integrate_channels1, kernel_size=3, padding=1)

        self.rconv = nn.Conv2d(mid_channels1 + 2*integrate_channels1, mid_channels2, kernel_size=1)

        # Middle convolutional layers2
        self.m2conv1 = nn.Conv2d(mid_channels2, mid_channels2, kernel_size=3, padding=1)
        self.m2conv2 = nn.Conv2d(mid_channels2, mid_channels2, kernel_size=3, padding=1)
        self.m2conv3 = nn.Conv2d(mid_channels2, mid_channels2, kernel_size=3, padding=1)
        self.m2conv4 = nn.Conv2d(mid_channels2, mid_channels2, kernel_size=3, padding=1)


        # Pre T convs
        self.ptconv2 = nn.Conv2d(mid_channels2, integrate_channels2, kernel_size=1)
        self.ptconv3 = nn.Conv2d(integrate_channels2, integrate_channels2, kernel_size=3, padding=1)
        self.ptconv4 = nn.Conv2d(integrate_channels2, integrate_channels2, kernel_size=3, padding=1)
        self.ptconv5 = nn.Conv2d(integrate_channels2, mid_channels2, kernel_size=1)

        # T convs
        self.tconv1 = nn.Conv2d(mid_channels2 + 2 * integrate_channels2, out_channels * (scale**2), kernel_size=1)
        ...
        # bT convs
        # self.btconv1 = nn.Conv2d(mid_channels, integrate_channels, kernel_size=1)
        self.btconv2 = nn.Conv2d(mid_channels2, integrate_channels2, kernel_size=1)
        self.btconv3 = nn.Conv2d(integrate_channels2, integrate_channels2, kernel_size=3, padding=1)
        self.btconv4 = nn.Conv2d(integrate_channels2, integrate_channels2, kernel_size=3, padding=1)
        self.btconv5 = nn.Conv2d(integrate_channels2, integrate_channels2, kernel_size=1)

        # aT convs
        # self.atconv1 = nn.Conv2d(mid_channels, integrate_channels, kernel_size=1)
        self.atconv2 = nn.Conv2d(mid_channels2, integrate_channels2, kernel_size=1)
        self.atconv3 = nn.Conv2d(integrate_channels2, integrate_channels2, kernel_size=3, padding=1)
        self.atconv4 = nn.Conv2d(integrate_channels2, integrate_channels2, kernel_size=3, padding=1)
        self.atconv5 = nn.Conv2d(integrate_channels2, integrate_channels2, kernel_size=1)
    ...
    def forward(self, lqs):
        ...
        feat_skip = x
        x = self.relu(self.m2conv1(x))
        x = self.relu(self.m2conv2(x))
        x = self.relu(self.m2conv4(x))
        x = x + feat_skip
...
if __name__ == '__main__':
    ...
    model = WGEN24VSR(mid_channels2=24, num_blocks=6)
    model.eval()
    ...
    # edge_model.export("/content/MIA-VSR/assets/wgen24vsr4.tflite")