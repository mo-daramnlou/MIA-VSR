...
class DSB(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_ds = nn.Conv2d(in_filters, 8, kernel_size=3, padding=1, stride=2)

        self.rconv= nn.Conv2d(out_filters + 8 , out_filters, kernel_size=1)

        self.relu = nn.ReLU(inplace=True)

    def forward(self, inputs):
        x = self.relu(self.conv(inputs))
        x_ds = self.relu(self.conv_ds(inputs))
...
class WGEN58VSR(nn.Module):
    def __init__(self, scale=4, in_channels=3, mid_channels=24, num_blocks=4, out_channels=3, integrate_channels=16):
        ...
        for _ in range(num_blocks):
            middle_layers.append(DSB(in_filters=mid_channels,out_filters=mid_channels))
...
if __name__ == '__main__':
    ...
    model = WGEN58VSR(mid_channels=24, num_blocks=4)
    model.eval()
