...
def gpu_paired_random_crop(img_gts, img_lqs, gt_patch_size, scale):
    """Paired random crop on GPU tensors."""
...
class REDSEffRollingCacheDataset(data.Dataset):
    def __init__(self, opt):
        super(REDSEffRollingCacheDataset, self).__init__()
        self.opt = opt
        ...
        logger.info(f'Dataset initialized with {len(self.all_clip_names)} total clips. Ready to load clips on-demand.')

    def clear_cache(self):
        """Explicitly clears the VRAM cache and forces garbage collection."""
        logger = get_root_logger()
    ...
    def load_clips_into_cache(self, clips_to_load):
        self.clear_cache()
        logger = get_root_logger()
        logger.info(f"----------- Loading {len(clips_to_load)} new clips into VRAM... -----------")
